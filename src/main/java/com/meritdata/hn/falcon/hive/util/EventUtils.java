...
package com.meritdata.hn.falcon.hive.util;

import com.meritdata.hn.falcon.hive.HiveDRArgs;
...
public class EventUtils {
    ...
    public void processEvents(String event) throws Exception {
        ...
        if (conf.get(HiveDRArgs.EXECUTION_STAGE.getName())
            ...
            if (StringUtils.isNotEmpty(exportEventStr)) {
                LOG.info("Process the export statements for db {} table {} exportEventStr {}", dbName, tableName, exportEventStr);
    ...
    /**
     * 在window环境中出现以下错误：
     * Error while compiling statement: FAILED: SemanticException Line 1:13 Invalid path ''49'': Target is not an empty directory
     * 次代码二次处理路径
     */
    private String reprocessSep(String input) {
        // String input = "EXPORT TABLE default.test2 TO '/apps/hive/tools/dr\\synclab\\49.default.test2.null.1915694208' FOR REPLICATION('49')";
        String regex = "TO.*?FOR"; // 匹配 TO 和 FOR 之间的内容
    ...
    private void executeSqlStatement(Command cmd, String dbName, String tableName,
            ...
            throws HiveReplicationException, SQLException, IOException {
        LOG.info("stmt:{}", stmt);
        String stmtForExec = reprocessSep(stmt);
        LOG.info("stmtForExec:{}", stmtForExec);
        try {
            sqlStmt.execute(stmtForExec);
    ...
    private void addReplicationStatus(ReplicationStatus.Status status, String dbName, String tableName, long eventId)
        ...
        try {
            ...
            String targetUri = conf.get(HiveDRArgs.TARGET_CLUSTER.getName());
            ReplicationStatus rs = new ReplicationStatus(conf.get(HiveDRArgs.SOURCE_CLUSTER.getName()),
                    targetUri, drJobName, dbName, tableName, status, eventId);
