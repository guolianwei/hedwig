...
import com.meritdata.hn.falcon.hive.exception.HiveReplicationException;
import com.meritdata.hn.falcon.hive.util.DRStatusStore;
import com.meritdata.hn.falcon.hive.util.DelimiterUtils;
import com.meritdata.hn.falcon.hive.util.FileUtils;
import com.meritdata.hn.falcon.hive.util.HiveDRUtils;
import com.meritdata.hn.falcon.hive.util.HiveMetastoreUtils;
...
public class LastReplicatedEvents {
    ...
    public HCatClient getTargetMetastoreClient() {
        return targetMetastoreClient;
    }

    public Path getEventsInputDirPath() {
        return eventsInputDirPath;
    }
    ...
    public LastReplicatedEvents(Configuration conf, String targetMetastoreUri,
                                String targetMetastoreKerberosPrincipal,
                                String targetHive2KerberosPrincipal,
                                DRStatusStore drStore, String jobName) throws Exception {
        targetMetastoreClient = HiveMetastoreUtils.initializeHiveMetaStoreClient(targetMetastoreUri,
                targetMetastoreKerberosPrincipal, targetHive2KerberosPrincipal);
        jobFS = FileSystem.get(conf);
        this.drStore = drStore;
    ...
    public String getLastEvents(HiveDROptions inputOptions) throws Exception {
        List<String> sourceTables = inputOptions.getSourceTables();
        List<String> sourceDatabases = inputOptions.getSourceDatabases();
        String confVal = targetMetastoreClient.getConfVal(HiveConf.ConfVars.METASTOREURIS.varname, "");

        HiveDRUtils.ReplicationType replicationType = HiveDRUtils.getReplicationType(sourceTables);
        LOG.info("Obtaining last events from : {} for replicationType : {}", confVal, replicationType);

        HashMap<String, Long> lastEvents = new HashMap<String, Long>();
        if (replicationType == HiveDRUtils.ReplicationType.DB) {
            List<String> dbNames = sourceDatabases;
    ...
    private long getLastSavedEventId(HiveDROptions inputOptions, final String dbName,
        ...
        return getLastSavedEventIdFromScalar
                (dbName, tableName, sourceTables, jobName, sourceMetastoreUri, targetMetastoreUri);
    }

    private long getLastSavedEventIdFromScalar(String dbName, String tableName, List<String> sourceTables,
                                               String jobName, String sourceMetastoreUri,
                                               String targetMetastoreUri) throws Exception {
        HiveDRUtils.ReplicationType replicationType = HiveDRUtils.getReplicationType(sourceTables);
        long eventId = 0;
        ...
        if (eventId == -1) {
            if (HiveDRUtils.ReplicationType.DB == replicationType) {
                ...
                try {
                    ...
                    String name = HiveConf.ConfVars.METASTOREURIS.varname;
                    String confVal = targetMetastoreClient.getConfVal(name, "");
    ...
    public String persistLastEventsToFile(final HashMap<String, Long> lastEvents,
        ...
        if (lastEvents.size() != 0) {
            ...
            String qualifiedEventsFile = jobFS.makeQualified(eventsFile).toString();
            LOG.info("eventsFile out {}", qualifiedEventsFile);
            return qualifiedEventsFile;
        } else {
            return null;
        }
    }
