...
static void main(String[] args) {
    String days = args[0]
    String HDFS_PATH = "/user/hive/warehouse/"
   // def configPath = "D:\\项目\\大数据任务监管平台配置库\\backup\\05_项目支持\\15_淮南数据灾备\\pjconf"
    String configPath=args[1]
    // 设置配置文件路径
    findModifyed(days, HDFS_PATH,configPath)
}


private void findModifyed(long days, String HDFS_PATH,String configPath) {
    def ten_days_ago = LocalDate.now().minusDays(days.longValue()).toString()
    ...
    fs.listStatus(new Path(HDFS_PATH)).each { FileStatus dbStatus ->
        if (dbStatus.getPath().getName().endsWith(".db")) {
            ...
            fs.listStatus(dbStatus.getPath()).each { FileStatus status ->
                // 获取文件或目录的修改日期
                def modification_date = new SimpleDateFormat("yyyy-MM-dd").format(new Date(status.getModificationTime()))
                total_scaned_count++
                // 检查 HDFS 路径是否为空
